...
namespace WebDataAgro.Controllers
{
    public class ReportePesificadosController : Controller
    {
        ...
        public ActionResult ListarProveedor(string text = "")
        {
            ...
            return Json(proveedores.Select(x => new { x.CUIT, Proveedor = x.RazonSocial }), JsonRequestBehavior.AllowGet);
        }
        public ActionResult ListarCorredor(string text = "")
        {
            var corredores = proveedorManager.ListarCorredor(text);
            return Json(corredores.Select(x => new { CuitCorredor = x.CUIT, Corredor = x.RazonSocial }), JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
