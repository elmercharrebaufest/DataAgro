...
function CreateGridZona() {
    $("#gridIni").kendoGrid({
        columns: [
            { field: "Descripcion", title: "Zona", filterable: false },
...
function InicializarBusquedaInicial() {
    ...
    MSExecuteURLOnServerAsync('/Zona/Buscar', funcReturn, '');
}
...
function UpdateViewModel(model) {
    ...
    var Zona = {
        ...
        "CodigoSap": model.Zona.CodigoSap,
    };

    viewModel.set("Zona", Zona);
}
...
function Agregar() {
    var result = MSExecuteURLOnServer('/Zona/Cancelar');
...
function Modificar() {
    ...
    if (data.Id > 0) {
        var datosZona = MSExecuteOnServer('/Zona/ZonaCombo', param);

        if (datosZona != null) {
            if (ExistsErrorMessages(datosZona.Errores)) {
                ShowTooltipMessages("err", datosZona.Errores);
            }
            else {
                viewModel.set("Zona", datosZona.Zona);
            }
        }
    }

    var result = MSExecuteOnServer('/Zona/Aplicar', param);
...
function EjecutarEliminar() {
    ...
    var result = MSExecuteOnServer('/Zona/Eliminar', param);
...
function Grabar() {
    ...
    var datos = {
        ...
        "Id": viewModel.get("Zona.Id"),
        "Descripcion": viewModel.get("Zona.Descripcion"),
        "CodigoSap": viewModel.get("Zona.CodigoSap"),
    };

    var result = MSExecuteOnServer('/Zona/Grabar', datos);
