...
document.getElementById("grano-filter").addEventListener("change", function () {
    var selectedGrano = this.value;
    console.log(selectedGrano);
    updateMarkers(selectedGrano);
});

function updateMarkers(selectedGrano) {

    // Filtra los marcadores según el grano seleccionado
    var filteredMarkers = addressPoints.filter(function (marker) {
        return marker.grano === selectedGrano || selectedGrano == "Todos";
    });
...
function cargarInformacionPopup(popup) {
    ...
    +'<div class="col-md-12"><span class="bold-text">Campania:</span> <span id="campania"></span></div>'
    ...
    document.getElementById("hubicacion").innerHTML = `${popup.options.provincia} ${popup.options.partido} ${popup.options.localidad}`;
