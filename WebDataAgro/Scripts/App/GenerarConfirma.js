    ...
    transport: {
      ...
      read: {
        ...
        data: function () {
          ...
          return {
            EsSoloPendientes: $("#ContratosPendientesCheck").is(":checked"),
            Filtros: filtroCompleto,
          };
        },
      },
    },
  ...
  $("#contratos-grid").kendoGrid({
    ...
    columns: [
      ...
      },
      {
        ...
        editable: false,
        type: "string",
        headerAttributes: { title: "Material" },
        filterable: {
          multi: true,
          dataSource: [
            { Material: "Maiz" },
      ...
      },
      {
        ...
        title: "Tipo Boleto",
        width: 60,
        headerAttributes: { title: "Tipo Boleto" },
        type: "string",
      },
      {
        field: "Bolsa",
      ...
      },
      {
        ...
        title: "Estado V.",
        width: 65,
        headerAttributes: { title: "Estado Versión" },
        editable: false,
        type: "string",
        filterable: {
          multi: true,
      ...
      },
      {
        ...
        width: 70,
        format: "{0:dd/MM/yyyy}",
        headerAttributes: { title: "Fecha Generación" },
        type: "date",
      },
      {
        ...
        width: 70,
        format: "{0:dd/MM/yyyy}",
        headerAttributes: { title: "Fecha Operación" },
        type: "date",
      },
      {
        field: "FechaConfirmacion",
        title: "F. Confirmación",
        width: 70,
        format: "{0:dd/MM/yyyy}",
        headerAttributes: { title: "Fecha Confirmación" },
        type: "date",
      },
      {
        ...
        width: 70,
        format: "{0:dd/MM/yyyy}",
        headerAttributes: { title: "Fecha Anulación" },
        type: "date",
      },
      {
        ...
        title: "C. Vendedor",
        width: 80,
        headerAttributes: { title: "Contrato Vendedor" },
        type: "string",
      },
      {
        ...
        title: "C. Corredor",
        width: 80,
        headerAttributes: { title: "Contrato Corredor" },
        type: "string",
      },
      {
        field: "Vendedor",
      ...
      },
      {
        ...
        filterable: {
          multi: true,
          dataSource: [{ Moneda: "USD" }, { Moneda: "ARP" }],
        },
      },
      {
        field: "TipoNegocio",
        title: "Tipo Contrato",
        width: 55,
        headerAttributes: { title: "Tipo Contrato" },
        editable: false,
        type: "string",
        filterable: {
          multi: true,
...
$("#NegocioSAP-desde").bind("paste", function (e) {
  //En caso de Pegar Codigos
  e.preventDefault();
...
function CargarTablaModal(contratos) {
  ...
  for (var i = 0; i < contratos.length; i++) {
    tabla +=
      ...
      "</td><td " +
      (contratos[i].TieneFechaGeneracionUltimoConfirma
        ? 'title="Último Confirma generado"'
        : "") +
      ">" +
      (contratos[i].FechaGeneracionFormateada == "1/1/0001"
        ? ""
        : contratos[i].FechaGeneracionFormateada) +
      "</td><td>" +
      (contratos[i].Generado
        ? '<i class="fa fa-check generado" aria-hidden="true" style="color:green; text-align: center"></i>'
        : '<i class="fa fa-times generado" aria-hidden="true" style="color:red; text-align: center"></i>') +
      "</td><td>" +
      (contratos[i].IsWebService
        ? '<i class="fa fa-check web" aria-hidden="true" style="color:green; text-align: center"></i>'
        : '<i class="fa fa-times generado" aria-hidden="true" style="color:red; text-align: center"></i>') +
      (contratos[i].Generado && descargaHabilitada
        ? '<a href="/Confirma/DescargarArchivoConfirma?nombreArchivo=' +
          contratos[i].Archivo +
          '" class="k-button k-button-icontext" style="height: 34px;text-align: center;margin-left: 1rem;"><i class="fa fa-download generado" style="text-align: center"></i></a>'
        : "") +
      "</td><td>" +
      contratos[i].Mensaje +
      "</td></tr>";
  ...
  setTimeout(function () {
    try {
      if (esContratoDesdeValido() && validarFechasYContratos()) {
        ...
        if (!esContratoDesdeValido())
          MensErr("Solo se admiten números y el ';' en el campo Contrato.");
        else
          MensErr(
            "El Rango de Contratos o Fechas no es válido. El campo Desde debe tener un valor menor al campo Hasta.",
          );
      }
    } catch (e) {
...
function trimEnd(cadena) {
  return cadena.at(cadena.length - 1) == ";"
...
function validarFechasYContratos() {
  var contratoDesde = (
    $("#NegocioSAP-desde").val().endsWith(";")
      ? $("#NegocioSAP-desde").val().slice(0, -1)
      : $("#NegocioSAP-desde").val()
  ).trim();
  var contratoHasta = $("#NegocioSAP-hasta").val().trim();

  if (contratoDesde == "" && contratoHasta == "") {
    var fechaDesde = $("#FechaConfirmacionDesde")
      .data("kendoDatePicker")
      .value();
    var fechaHasta = $("#FechaConfirmacionHasta")
      .data("kendoDatePicker")
      .value();
...
function GenerarConfirma(negocioSAP) {
  ...
  setTimeout(function () {
    try {
      ...
      if (
        result.confirmasGenerados == undefined ||
        result.confirmasGenerados.length == 0
      ) {
        if (result.HayError) {
...
function GestionarClausulas(negocioSAP) {
  ...
  if (response && response.Mensaje == "") {
    ...
    MensErr(
      response.Mensaje || "Ocurrió un error al intentar validar el Negocio.",
    );
  }
}
