        ...
        private static List<string[]> Query(DbContext contexto,int materialId, int mes, bool? calidad)
        {
            ...
            return contexto.Set<Contrato>().Where(x => DbFunctions.TruncateTime(x.Fecha) == fechaHoy && (x.EstadoId == 2 || x.EstadoId == 4 || x.EstadoId == 5) && x.MaterialId == materialId && SqlFunctions.DatePart("Month", x.FechaHasta) == mes && (calidad == null || (calidad != null && x.TrigoEspecial == calidad))).DefaultIfEmpty()
                       .Select(x => new List<string>
                       {
                           ...
                           SqlFunctions.DateName("day", x.FechaDesde) != null ? SqlFunctions.DateName("day", x.FechaDesde) + "/" + SqlFunctions.DatePart("month", x.FechaDesde) + "/" + SqlFunctions.DateName("year", x.FechaDesde) : "",
                           SqlFunctions.DateName("day", x.FechaHasta) != null ? SqlFunctions.DateName("day", x.FechaHasta) + "/" + SqlFunctions.DatePart("month", x.FechaHasta) + "/" + SqlFunctions.DateName("year", x.FechaHasta) : "",
                           x.Precio.ToString() ?? "",
                           x.Moneda.Descripcion ?? "",
                           SqlFunctions.DateName("day", x.Fecha) != null ? SqlFunctions.DateName("day", x.Fecha) + "/" + SqlFunctions.DatePart("month", x.Fecha) + "/" + SqlFunctions.DateName("year", x.Fecha) : "",
                           x.Provincia.Nombre ?? "",
                           x.Localidad.Nombre ?? "",
                           ...
                           x.DesdeFijacion.HasValue ? SqlFunctions.DateName("day", x.DesdeFijacion) + "/" + SqlFunctions.DatePart("month", x.DesdeFijacion) + "/" + SqlFunctions.DateName("year", x.DesdeFijacion) : "",
                           x.HastaFijacion.HasValue ? SqlFunctions.DateName("day", x.HastaFijacion) + "/" + SqlFunctions.DatePart("month", x.HastaFijacion) + "/" + SqlFunctions.DateName("year", x.HastaFijacion) : "",
                           x.Base == true ? "X" : "",
                           x.ImporteSustentable.HasValue && x.MonedaSustentableId != null ? x.ImporteSustentable.ToString() + " " + x.MonedaSustentable.Descripcion : "",
                           x.FechaDolarizado!=null ? SqlFunctions.DateName("day", x.FechaDolarizado) + "/" + SqlFunctions.DatePart("month", x.FechaDolarizado) + "/" + SqlFunctions.DateName("year", x.FechaDolarizado) : "",
