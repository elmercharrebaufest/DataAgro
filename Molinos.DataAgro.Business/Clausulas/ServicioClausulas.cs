...
namespace Molinos.DataAgro.Business.Clausulas
{
    public class ServicioClausulas : IServicioClausulas
    {
        ...
        public ServicioClausulas(ILogger log)
        {
            this.log = log;
            RegistrarProcesadores();
        }

        public ResultadoClausula DevolverClausulas(Clausula clausula)
        {
            var tipoProcesador = procesadores[clausula.GetType()];
            var procesador = (IProcesadorClausula)System.Web.Mvc.DependencyResolver.Current.GetService(tipoProcesador);
        ...
        private void RegistrarProcesadores()
        {
            ...
            var clausulas = Clausula.TiposDeComandos().Where(t => !t.IsAbstract);
        ...
        private Type ObtenerProcesador(Type clausula)
        {
            try
            {
                return typeof(IProcesadorClausula<>)
                    .Assembly
                    .GetExportedTypes()
                    .Single(
                        x => !x.IsAbstract && x.GetInterfaces().Any(i => i.IsGenericType
                                                        && i.GetGenericTypeDefinition() == typeof(IProcesadorClausula<>)
