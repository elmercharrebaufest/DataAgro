        ...
        public override ResultadoClausula DevolverClausulas(ClausulaTres clausula)
        {
            ...
            else if ((clausula.Basico.TipoNegocioId == (int)EnumTipoNegocio.A_FIJAR && clausula.Basico.Canje != true) || esFijacionContratoConvenio /* || (esFijacionConContratoCanje)*/)
            {
                ...
                if ((clausula.Basico.TipoNegocioId == (int)EnumTipoNegocio.FIJACION && contrato.Canje == true) || clausula.Basico.TipoNegocioId == (int)EnumTipoNegocio.A_FIJAR)
                {
                    ...
                    res.Texto += $", con mercadería descargada en fábrica, liquidándose el 2.5% (dos y medio por ciento) restando a los 30 (treinta) días del cumplimiento del contrato. “El precio de la mercadería objeto del presente contrato, se fijará cualquier día hábil a elección del vendedor." +
                    $" El vendedor comunicará al comprador el día elegido para la fijación de precio por Mercado Disponible de Molinos Agro SA hasta el {contrato.FechaHasta.ToString("dd/MM/yyyy", CultureInfo.InvariantCulture)}" +
                    $" cualquier día hábil a elección del vendedor”. (También la fijación puede ser por Pizarra Rosario Ciega o por ambas). " +
                    $"“A los efectos del impuesto de sellos, únicamente, las partes acuerdan que el precio de referencia corresponde a Pizarra Rosario Soja/Ciega”. Las Partes acuerdan la posibilidad de prorrogar el plazo de pago indicado en las cláusulas previas, a opción del Vendedor. " +
