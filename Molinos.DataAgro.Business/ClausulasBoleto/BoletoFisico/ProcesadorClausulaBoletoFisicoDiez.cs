...
{
    public class ProcesadorClausulaBoletoFisicoDiez : ProcesadorClausulaBoletoFisico<ClausulaBoletoFisicoDiez>
    {
        public ProcesadorClausulaBoletoFisicoDiez(IRepositorio repositorio, ILogger log, IConsultarEstadoBoletoAgent estadoBoleto)
            : base(repositorio, log, estadoBoleto) { }

        public override ResultadoClausula DevolverClausulas(ClausulaBoletoFisicoDiez clausula)
        {
            ...
            if (clausula.Basico.TipoNegocioId == (int)EnumTipoNegocio.A_FIJAR && (!clausula.Basico.TipoPosicionCBOTId.HasValue || clausula.Basico.TipoPosicionCBOTId.Value != (int)EnumTipoPosicionCBOT.PASE) && clausula.Basico.EsFason != true && clausula.Basico.PrestamoDevolucion != true)
            {
             ...
             $"{MetodosUtiles.CorregirFormatoFecha(condiciones.FechaDesde)} hasta el {MetodosUtiles.CorregirFormatoFecha(condiciones.FechaHasta)} en cualquier día hábil a elección del vendedor, siendo la cantidad de " +
             $"{condiciones.Meins} de fijación mínima permitida {MetodosUtiles.NumeroConSeparadores(condiciones.CantidadMinima)} Kg. y la cantidad máxima permitida {MetodosUtiles.NumeroConSeparadores(condiciones.CantidadMaxima)} Kg.";
