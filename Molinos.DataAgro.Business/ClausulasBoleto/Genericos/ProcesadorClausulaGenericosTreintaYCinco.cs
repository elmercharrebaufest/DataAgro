...
namespace Molinos.DataAgro.Business.ClausulasBoleto.Genericos
{
    public class ProcesadorClausulaGenericosTreintaYCinco : ProcesadorClausulaGenericos<ClausulaGenericosTreintaYCinco>
    {
        ...
        public override ResultadoClausula DevolverClausulas(ClausulaGenericosTreintaYCinco clausula)
        {
            ...
            {
                res.Texto += $" Las Partes establecen que, para el caso que al momento de la entrega se constate que la mercadería entregada no cumple con los requisitos de soja SUSTENTABLE, el precio convenido será bonificado en la cantidad de USD {clausula.Basico.Importe_Sustentable?.ToString("N", new CultureInfo("es-AR"))} ({MetodosUtiles.DevolverNumeroEnLetrasConDivisa((decimal)clausula.Basico.Importe_Sustentable, "USD")}) por tonelada de soja no SUSTENTABLE, a favor del Comprador. Dicha bonificación será descontada al momento del pago de cada una de las liquidaciones parciales y/o finales según corresponda.\"";
            }
            ...
            {
                res.Texto += $" Las Partes establecen que, para el caso que al momento de la entrega se constate que la mercadería entregada no cumple con los requisitos de soja SUSTENTABLE, el precio convenido será bonificado en la cantidad de $ {clausula.Basico.Importe_Sustentable?.ToString("N", new CultureInfo("es-AR"))} ({MetodosUtiles.DevolverNumeroEnLetrasConDivisa((decimal)clausula.Basico.Importe_Sustentable, "ARP")}) por tonelada de soja no SUSTENTABLE, a favor del Comprador. Dicha bonificación será descontada al momento del pago de cada una de las liquidaciones parciales y/o finales según corresponda.\"";
            }
            ...
            {
                res.Texto = $"Las Partes establecen que, para el caso que al momento de la entrega se constate que la mercadería entregada no cumple con las especificaciones y certificaciones establecidas para la Soja Sustentable \"EPA\" (certificación de establecimiento e indicación \"EPA\" en Observaciones de la Carta de Porte) y condiciones de entrega (fecha de entrega desde {clausula.Basico.FechaDesde.GetValueOrDefault():dd'.'MM'.'yyyy} hasta {clausula.Basico.FechaHasta.GetValueOrDefault():dd'.'MM'.'yyyy} y código de cupo), el precio será bonificado en la cantidad de $ {clausula.Basico.Importe_Sustentable.GetValueOrDefault().ToString("N", new CultureInfo("es-AR"))} ({MetodosUtiles.DevolverNumeroEnLetrasConDivisa((decimal)clausula.Basico.Importe_Sustentable.GetValueOrDefault(),"ARP")}) por tonelada, a favor del Comprador. Dicha bonificación será descontada al momento del pago de cada una de las liquidaciones parciales y/o finales según corresponda.";
            }
