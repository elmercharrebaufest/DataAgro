using Molinos.DataAgro.Entities.Dto;
using Molinos.DataAgro.Interfaces;
using Molinos.DataAgro.Repository;
using System;
using NLog;
using System.Globalization;
using System.Linq;
using System.Text;
using System.Threading;
using System.Threading.Tasks;
using Molinos.DataAgro.Entities.ClausulasBoleto;
using Molinos.DataAgro.Entities.Entities;
namespace Molinos.DataAgro.Business.ClausulasBoleto.Confirma
{
    public abstract class ProcesadorClausulaConfirma<TClausula> : IProcesadorClausulaConfirma<TClausula> where TClausula : ClausulaConfirma
    {
        protected IRepositorio Repositorio { get; private set; }
        protected ILogger Log { get; private set; }
        public IConsultarEstadoBoletoAgent EstadoBoleto { get; }

        protected ProcesadorClausulaConfirma(IRepositorio repositorio, ILogger log, IConsultarEstadoBoletoAgent estadoBoleto)
        {
            Log = log;
            EstadoBoleto = estadoBoleto;
            Repositorio = repositorio;
        }

        public virtual ResultadoClausula DevolverClausulas(TClausula comando) { return new ResultadoClausula(); }


        public ResultadoClausula DevolverClausulas(ClausulaConfirma comando)
        {
            var count = 1;
            const int maxTries = 3;
            Humanizer.Configuration.Configurator.Ordinalizers.ResolveForCulture(CultureInfo.GetCultureInfo("es-AR"));
            while (true)
            {
                try
                {
                    return DevolverClausulas((TClausula)comando);
                }
                catch (Exception e)
                {
                    Thread.Sleep(count * 1500);
                    Log.Error(e, String.Format("Ocurrió un error al ejecutar la ClausulaConfirma - Intento número:" + count));
                    if (++count == maxTries)
                    {
                        throw;
                    }
                }
            }
        }

    }
}
