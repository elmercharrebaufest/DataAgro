    ...
    public class LogDataAgroManager : ILogDataAgroManager
    {
        ...
        private string ObtenerUsuario()
        {
            ...
            try
            {
                ...
                usuarioEncontrado = PermisosHelper.ObtenerUsuario();
            ...
            {
                var comercial = repositorio.Obtener<Comercial, string>(x => x.IdActiveDirectory == usuarioEncontrado, x => x.Nombres + " " + x.Apellido);
                if (comercial != null)
                {
                    usuarioEncontrado = comercial;
                }
            }
            else
            {
                usuarioEncontrado = "DATAAGRO";
            }
        ...
        private void Agregar(List<DatoModificadosLogDataAgroDto> cambiados, LogDataAgroDto logActual, JToken item2, string campo, JToken item)
        {
            ...
            var Anterior = BuscaFechaYFormatea(((JProperty)item2).First.ToString(), name);
            var Actual = BuscaFechaYFormatea(((JProperty)item2).Last.ToString(), name);
        ...
        void AgregarItem(List<DatoModificadosLogDataAgroDto> cambiados, LogDataAgroDto logActual, JToken item, string campo)
        {
            ...
            var Anterior = BuscaFechaYFormatea(((JContainer)((JProperty)item).First).First.ToString(), campo);
            var Actual = BuscaFechaYFormatea(((JContainer)((JProperty)item).First).Last.ToString(), campo);
                ...
                if (((JProperty)item).Name != null && (!((JProperty)item).Name.ToLower().EndsWith("id")) && !((JProperty)item).Name.ToLower().Contains("formateado"))// saco los dis
                {
                    ...
                    if (!((JProperty)item).Value.ToString().StartsWith("["))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Anterior = "",
                                Actual = BuscaFechaYFormatea(((JProperty)item).Value.ToString(), ((JProperty)item).Name),
                                Campo = campo
                            };
                            cambiados.Add(modificado);
                        }
