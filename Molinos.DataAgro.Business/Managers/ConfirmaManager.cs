    ...
    public class ConfirmaManager : IConfirmaManager
    {
        ...
        public byte[] ConfirmaEnByte(string codigoSAP, List<int> equipo)
        {
            try
            {
                ...
                logger.Info($"Otros Datos del Negocio de Confirma; Codigo:{contrato.FijacionSAP ?? contrato.ContratoSAP}, esCanje:{esCanje}, esConvenio:{esConvenio}, estadoSAP:NroSIO:{estadoSAP.NumeroSio} Partes: { string.Join(" - ", Partes.Select(e => "NroInterno: "+e.NroContratoInterno+" Cuit:"+e.CUIT))}, estadoSAP: {estadoSAP.NumeroSio}, condiciones fijacion: CantidadMaxima:{condiciones.CantidadMaxima} . CantidadMinima{condiciones.CantidadMinima}");
