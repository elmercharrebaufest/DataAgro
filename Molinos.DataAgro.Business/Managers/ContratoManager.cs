...
namespace Molinos.DataAgro.Business.Managers
{
    public class ContratoManager : IContratoManager
    {
        ...
        private bool ConfirmacionAutomatica(Contrato contrato)
        {
            ...
            var rango = repositorio.Listar<RangoConfirmacionAutomatica>(x =>
            x.FechaDesde <= hoy &&
            x.FechaHasta >= hoy &&
            x.MaterialId == contrato.MaterialId &&
            x.MonedaId == contrato.MonedaId &&
            precioContrato >= x.PrecioMinimo && precioContrato <= x.PrecioMaximo).FirstOrDefault(x => contrato.FechaDesde >= new DateTime(x.DesdeAnio, x.DesdeMes, 1) &&
