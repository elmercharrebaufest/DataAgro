    ...
    public class CupoManager : ICupoManager
    {
        ...
        public List<ExcelValidatorResumeItem> AltaMasivaSugerenciaCuposV2(DataSet dsExcel)
        {
            ...
            var ExcelData = dsExcel.Tables[0].AsEnumerable()
                  ...
                  {
                      ContratoSAP = row["ContratoSAP"]?.ToString().PadLeft(10, '0'),
                      FechaSugerida = row["FechaSugerida"] is DateTime dt
                            ? dt
                            : double.TryParse(row["FechaSugerida"]?.ToString(), out var oa)
                                ? DateTime.FromOADate(oa)
                                : DateTime.Parse(row["FechaSugerida"]?.ToString()),
                      CantidadDeCupos = int.Parse(row["CantidadDeCupos"]?.ToString()),
                  })
                  .Where(x => !string.IsNullOrEmpty(x.ContratoSAP))
                  .ToList();
        ...
        public List<SugerenciaCupoDto> CrearSugerenciaCupoMasiva(int MaterialId, FormulaDto formula, List<Negocio> NegociosAUsar, List<AltaMasivaCupoDto> ExcelData)
        {
            try
            {
                DateTime hoy = DateTime.Now.Date;
                ...
                logger.Debug("CrearSugerenciaCupo - inicio de PriorizarSegunDisponibilidad.");
                PriorizarSegunDisponibilidadAltaMasiva(disponibilidadEnPlantas, negocios);
                logger.Debug("CrearSugerenciaCupo - negocios priorizados: " + negocios.Where(a => a.Priorizado).Count());
                logger.Debug("CrearSugerenciaCupo - fin de PriorizarSegunDisponibilidad.");
        ...
        private void PriorizarSegunDisponibilidadAltaMasiva(List<ConfiguracionCupoDto> disponibilidadEnPlantas, List<SugerenciaCupoDto> negocios)
        {
            var idsProveedores = negocios.Select(a => a.ProveedorId ?? 0).Distinct();
            // Cuanto mas cerca a cero, mas equitativa la distribución. No se tomará de AlgoritmoProcMaxSugerenciasProveedorDia
            // para tener independencia respecto al Algoritmo.
            int porcentajeMaximo = 1;
            List<TopeSugerenciasPorDiaPorProveedor> limitePorProveedor = new List<TopeSugerenciasPorDiaPorProveedor>();
            ...
            {
                hayDisponibilidad = false;
                var negociosOrdenados = negocios.OrderBy(a => a.FechaHastaOriginal).ThenBy(a => a.ContratoSAP);

                foreach (var negocio in negociosOrdenados)
                {
                    var disponibles = disponibilidadEnPlantas.Where(a => a.MaterialId == negocio.MaterialId && a.LimiteAlgoritmo > 0).OrderBy(a => a.Fecha).ToList();
        ...
        // ============================================================================
        // MÉTODO CON COMENTARIOS: ObtenerNegocios
        // ============================================================================
        // PROPÓSITO: Recupera y procesa contratos finalizados para generar sugerencias 
        // automáticas de cupos, aplicando múltiples validaciones y descuentos.
        // 
        // ENTRADA: 
        //   - hoy: Fecha actual (DateTime)
        //   - formula: Configuración actual con rangos de fechas y filtros (FormulaDto)
        //   - negocios: Lista acumulativa de sugerencias (se modifica por referencia)
        //
        // RETORNO: Negocios que quedaron sin cantidad de cupos sugerible (inhabilitados)
        // ============================================================================
        private List<SugerenciaCupoDto> ObtenerNegocios(DateTime hoy, FormulaDto formula, List<SugerenciaCupoDto> negocios)
        {
            ...
            // Obtiene los tipos de negocio que están marcados como excluidos para esta fórmula
            List<int> idsTiposNegociosExcluidos = repositorio.Listar<FormulaTipoNegocioExcluido, int>(
                x => x.TipoNegocioId,
                x => x.FormulaId == formula.Id
            );
            ...
            var contratos = repositorio.Listar<Contrato, SugerenciaCupoDto>(x =>
                ...
                x =>
                    ...
                    // Excluir tipos de negocio configurados como excluidos
                    !idsTiposNegociosExcluidos.Any(a => a == x.TipoNegocioId) &&
                    // Validar que el contrato esté dentro del rango de fechas de negocio
                    // (usa FechaHastaOriginal si existe, sino usa FechaHasta)
        ...
        // ============================================================================
        // MÉTODO CON COMENTARIOS: ObtenerNegociosAltaMasiva
        // ============================================================================
        // PROPÓSITO: Recupera y procesa contratos finalizados para generar sugerencias 
        // automáticas de cupos, aplicando múltiples validaciones y descuentos.
        // 
        ...
        private CupoResult CrearCupoSugerenciaDetalle(SugerenciaCupo s, SugerenciaPorComercial sugerenciaPorComercial, int cantidadACrear, bool esFlete, DateTime? fechaCupo, int? comercialCreador)
        {
            ...
            Cupo cupo = new Cupo
            {
                ...
                ComercialId = s.ComercialId,
                Calidad = s.StandardDeCalidad == null ? null : s.StandardDeCalidad == "Cámara" || s.StandardDeCalidad == "Camara" ? "Camara" : "Fabrica",
            ...
            try
            {
                ...
                foreach (var material in materiales)
                {
                    ...
                    var administracionCupos = repositorio.Listar<AdministracionCupo, AdministracionCupoDto>(x => new AdministracionCupoDto
                    {
                        ...
                        CantidadFleteProcedencia = x.CantidadFleteProcedencia,
                    ...
                    foreach (var fecha in fechasComprendidas)
                    {
                        if (fecha >= formula.CuposDesde && fecha <= formula.CuposHasta)
                        {
                            ...
                            var cupo = new DiaCupo()
                            {
                                ...
                                disponibilidadEnPlanta.Where(x => x.Fecha == fecha && x.MaterialId == material.MaterialId).Sum(y => y.LimiteAlgoritmo) -
                                disponibilidadEnPlanta.Where(x => x.Fecha == fecha && x.MaterialId == material.MaterialId).Sum(y => y.LimiteDescarga)) +
                                administracionCupos.Where(x => x.Fecha == fecha && !x.Excedente && x.MaterialId == material.MaterialId).Sum(x => x.CantidadCupo + x.CantidadFleteProcedencia) -
                                CantidadCuposGeneradosDesdeSolicitudYFueraDelAlgoritmo,

                                //Sugerencias Aceptadas
                                CantidadSugerenciaAceptadaDia = sugerenciaCupo.Where(x => x.FechaSugerida == fecha && x.CentroId == formula.CentroId && x.Aceptado == true && x.MaterialId == material.MaterialId).Sum(x => x.CantidadDeCupos),
                                //Sugerencias Devueltas
                                CantidadCuposDevueltos = administracionCupos.Where(x => x.Fecha == fecha && !x.Excedente && x.MaterialId == material.MaterialId).Sum(x => x.CantidadCupo + x.CantidadFleteProcedencia),
                                //Solicitudes Aceptadas
                                CantidadSolicitudesAceptadas = cupos.Count(x => x.FechaIngreso == fecha && x.CentroId == formula.CentroId && x.MaterialId == material.MaterialId && x.AdministracionCupoId != null && (x.EstadoCupoId != 4 && x.EstadoCupoId != 9)),
                                //Solicitudes Pendientes del algoritmo y extraordinarias.
                                CantidadSolicitudesPendientes = administracionCupos.Where(x => x.Fecha == fecha && x.Excedente && x.EstadoId == (int)EnumEstadoAdministracionCupo.Pendiente && x.MaterialId == material.MaterialId && x.TipoAdministracionCupoId == (int)EnumTipoAdministracionCupo.Algoritmo)
                                .Sum(x => x.CantidadCupo + x.CantidadFleteProcedencia),
                                //Solicitudes Pendientes Extra
                                CantidadSolicitudesPendientesExtra = administracionCupos.Where(x => x.Fecha == fecha && x.Excedente && x.EstadoId == (int)EnumEstadoAdministracionCupo.Pendiente && x.MaterialId == material.MaterialId && x.TipoAdministracionCupoId == (int)EnumTipoAdministracionCupo.Extraordinaria)
                                .Sum(x => x.CantidadCupo + x.CantidadFleteProcedencia),


                                CantidadSugerenciaPendiente = sugerenciaCupo.Where(x => x.FechaSugerida == fecha && x.CentroId == formula.CentroId && x.Aceptado == null
        ...
        public byte[] GenerarExcelNegociosAlgoritmo(List<SugerenciaCupoExcel> oDatos, List<FormulaDto> formulas, bool esCargaMasiva = false)
        {
        ...
        public List<NegocioParaSolicitarCupo> ListarNegociosParaSolicitarCupo(string contratoSap, int proveedorId, int materialId, int estadoId, bool sustentable, bool epa, bool eudr)
        {
            ...
            if (string.IsNullOrEmpty(contratoSap))
            {
                ...
                }, x => (x.ProveedorId == proveedorId || x.CorredorId == proveedorId) && x.MaterialId == materialId && x.Cantidad > 0
                    && x.TipoNegocioId != (int)EnumTipoNegocio.FIJACION && x.TipoNegocioId != (int)EnumTipoNegocio.CONTRATO_ACUERDO &&
