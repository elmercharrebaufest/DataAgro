        ...
        public ConfirmaAltaLoteResultDto ConfirmaLoteDocumentos(List<ResultadoClausula> clausulas, List<int> equipo, BasicoContrato contrato, EstadosConfirmaDto estadosConfirmaDto)
        {
            ...
            else
            {
                try
                {
                    ...
                    object item1 = null;
                    if (false)
                        item1 = DevolverItemDocumentoConsignacionFijarPrecio(contrato, clausulas, Partes, esCanje, esConvenio, condiciones, estadoSAP);
                    else if (false)
                        item1 = DevolverItemDocumentoConsignacionPrecioHecho(contrato, clausulas, Partes, esCanje, esConvenio, condiciones, estadoSAP);
                        ...
                        item1 = DevolverItemDocumentoFijarPrecioRofex(contrato, clausulas, Partes, esCanje, esConvenio, condiciones, estadoSAP);
                    else if ((contrato.TipoNegocioId == (int)EnumTipoNegocio.FIJACION || contrato.TipoNegocioId == (int)EnumTipoNegocio.A_FIJAR) && esCanje)
                        /* tiene Insumos */
                    ...
                    //else if (ContratoOIVFijarPrecioRofex) item1 = DevolverItemDocumentoOIVPrecioFijarRofex(contrato, clausulas, Partes, esCanje, esConvenio, condiciones, estadoSAP);
                    //else if (ContratoWSOIVPrecioFijar) item1 = DevolverItemDocumentoOIVFijarPrecio(contrato, clausulas, Partes, esCanje, esConvenio, condiciones, estadoSAP);
                    //else if (ContratoWSOIVPrecioHecho) item1 = DevolverItemDocumentoOIVPrecioHecho(contrato, clausulas, Partes, esCanje, esConvenio, condiciones, estadoSAP);

                    #region CodigoComentado
                    //DocumentoFijarPrecio item1 = new DocumentoFijarPrecio(); // Este "item" puede ser de muchos tipos. Ver a cual corresponde para saber que estructura usar.
                    ...
                    //            TCaption cantidadDesde = new TCaption();
                    //            cantidadDesde.Value = contrato.KgMinimo > 0 ? contrato.KgMinimo.ToString() : ((int)contrato.Cantidad).ToString();
                    //            detalleContrato.CantidadDesde = cantidadDesde;

                    //            TCaption cantidadHasta = new TCaption();
                    //            cantidadHasta.Value = contrato.KgMaximo > 0 ? contrato.KgMaximo.ToString() : ((int)contrato.Cantidad).ToString();
                    ...
                    //            #region Calidad
                    //                TCodLista condicionesCalidad = new TCodLista();
                    //                condicionesCalidad.CodLista = (contrato.StandardDeCalidadId == (int)EnumStandarCalidad.CAMARA || contrato.StandardDeCalidadId == (int)EnumStandarCalidad.ESPECIAL) ? "1" : (contrato.StandardDeCalidadId == (int)EnumStandarCalidad.FABRICA ? "4" : "");

                    //                TCaption otrasCondicionesCalidad = new TCaption();
                    //                otrasCondicionesCalidad.Value = string.Empty;

                    //                calidad.CondicionesCalidad = condicionesCalidad;
                    //                calidad.OtrasCondicionesCalidad = otrasCondicionesCalidad;
                    //            #endregion Calidad
                    ...
                    //            #region Entrega
                    //                TCaption entregaDesde = new TCaption();
                    //                entregaDesde.Value = contrato.FechaDesde.HasValue ? contrato.FechaDesde.Value.ToString("dd/MM/yyyy") : null;

                    //                TCaption entregaHasta = new TCaption();
                    //                entregaHasta.Value = contrato.FechaHasta.HasValue ? contrato.FechaHasta.Value.ToString("dd/MM/yyyy") : null;

                    //                entregas.EntregaDesde = entregaDesde;
                    //                entregas.EntregaHasta = entregaHasta;
                    //            #endregion Entrega
